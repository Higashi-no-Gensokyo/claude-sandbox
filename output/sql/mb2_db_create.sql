...
DROP TABLE IF EXISTS culture_role_refs;
...
CREATE TABLE kingdom_relationships (
  ...
  clan        VARCHAR(255) NULL     COMMENT '書式: Faction.xxx',
  kingdom_ref VARCHAR(255) NULL     COMMENT 'XSD属性名はkingdom。kingdom_id(FK)との衝突回避のためリネーム。書式: Kingdom.xxx',
  value       INT          NOT NULL,
  isAtWar     BOOLEAN      NULL     COMMENT 'XSD属性名はcamelCase',
  PRIMARY KEY (id),
...
CREATE TABLE culture_role_refs (
  ...
  ref_id     VARCHAR(255) NOT NULL COMMENT '書式: NPCCharacter.xxx または PartyTemplate.xxx（role_nameにより異なる）',
